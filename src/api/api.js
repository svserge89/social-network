...
export const authAPI = {
  ...
  login: (email, password, rememberMe, captcha) => (
    instance.post(AUTH_LOGIN, {email, password, rememberMe, captcha}).then(responseData)
  ),
...
export const usersAPI = {
  get: (count, page) => instance.get(USERS, {params: {count, page}}).then(responseData),

  follow: (userId) => instance.post(`${FOLLOW}/${userId}`).then(responseData),

  unFollow: (userId) => instance.delete(`${FOLLOW}/${userId}`).then(responseData)
};

export const profileAPI = {
  get: (userId) => instance.get(`${PROFILE}/${userId}`).then(responseData),

  update: (profile) => instance.put(PROFILE, profile).then(responseData),

  getStatus: (userId) => instance.get(`${PROFILE_STATUS}/${userId}`).then(responseData),

  updateStatus: (status) => instance.put(PROFILE_STATUS, {status}).then(responseData),
