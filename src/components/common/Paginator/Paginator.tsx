...
const Paginator: React.FC<PaginatorProps> = ({
  ...
  const createEllipsis = (page: number): JSX.Element => (
    <Pagination.Ellipsis key={page} onClick={clickHandler(page)} disabled={disabled}/>
  );

  const createFirst = (): JSX.Element => (
    <Pagination.First onClick={clickHandler(1)} disabled={disabled || currentPage <= 1}/>
  );

  const createPrev = (): JSX.Element => (
    <Pagination.Prev onClick={clickHandler(currentPage - 1)}
                     disabled={disabled || currentPage <= 1}/>
  );

  const createNext = (): JSX.Element => (
    <Pagination.Next onClick={clickHandler(currentPage + 1)}
                     disabled={disabled || currentPage >= pagesCount}/>
  );

  const createLast = (): JSX.Element => (
    <Pagination.Last onClick={clickHandler(pagesCount)}
                     disabled={disabled || currentPage >= pagesCount}/>
  );
