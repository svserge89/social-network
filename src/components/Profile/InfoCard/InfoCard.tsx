...
const InfoCard: React.FC<InfoCardProps> = ({editable = false}) => {
  ...
  const showEditButton = (): JSX.Element | '' =>
    editable ? (
      <ButtonWithIcon onClick={editHandler} icon={faEdit}>
        Edit
      </ButtonWithIcon>
    ) : (
      ''
  ...
  const showBody = (): JSX.Element =>
    !editMode ? (
      <Card.Body>
        <Card.Text as="div">
          <h5>
            <ComponentWithIcon icon={faAddressCard}>Contacts</ComponentWithIcon>
          </h5>
          <ContactList contacts={contacts} labels={contactLabels} />
        </Card.Text>
        <LookingForAJob
          answer={lookingForAJob}
          description={lookingForAJobDescription}
        />
        <Card.Text as="div">
          <h5>
            <ComponentWithIcon icon={faBook}>About me</ComponentWithIcon>
          </h5>
          {showAboutMe()}
        </Card.Text>
        {showEditButton()}
